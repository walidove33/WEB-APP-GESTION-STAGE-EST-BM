...
export class StageService {
  ...
  createPlanification(planificationData: any): Observable<PlanificationSoutenanceResponse> {
    ...
    const payload = {
      ...
      encadrantId: Number(planificationData.encadrant.id),
      departementId: Number(planificationData.departement.id),
      classeGroupeId: Number(planificationData.classeGroupe.id),
      anneeScolaireId: Number(planificationData.anneeScolaire.id),
    }

    console.log("📤 Sending payload:", payload)

    return this.http.post<PlanificationSoutenanceResponse>(`${this.baseUrl}/planification/create`, payload).pipe(
      tap((response) => {
